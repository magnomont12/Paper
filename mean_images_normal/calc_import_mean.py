...
def calc_distances_original():
    ...
    for a in range(len(matrix)):
        main_element =os.path.join("mean_images_normal","grad-cam",f"{matrix[a][a]}.png")
        main_element = cv2.imread(main_element)
        for b in range(len(matrix[a])):
            element = os.path.join("mean_images_normal","grad-cam",f"{matrix[a][b]}.png")
    ...
    for b in range(len(matrix[0])):
        main_element = os.path.join("mean_images_normal","grad-cam",f"{matrix[b][b]}.png")
        main_element = cv2.imread(main_element)
    ...
    matrix_h = pd.DataFrame(np.array(matrix_h)).to_csv("mean_images_normal/matrix_h_step1.csv")
    matrix_v = pd.DataFrame(np.array(matrix_v)).to_csv("mean_images_normal/matrix_v_step1.csv")
...
def calc_distance_blends():
    ...
    for model in models:
        ...
        for a in range(len(matrix_blend)):
            main_element =os.path.join("mean_images_normal","grad-cam",f"{matrix_blend[a][0]}.png")
            main_element = cv2.imread(main_element)
            for b in range(len(matrix_blend[a])):
                element = os.path.join("mean_images_normal","grad-cam",f"{matrix_blend[a][b]}.png")
                element = cv2.imread(element)
                distance = calc_pearson_correlation(main_element, element)
                matrix_h[a][b] = distance
        matrix_h[0].append(None)
        matrix_h[1].append(None)
        matrix_h = pd.DataFrame(np.array(matrix_h)).to_csv(f"mean_images_normal/matrix_h_step_{model}.csv")
